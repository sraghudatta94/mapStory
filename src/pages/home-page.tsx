...
const HomePage: React.FC<HomeProps> = () => {
    ...
    const selectedLocation = useMemo(() => {
        ...
        const locationIndex = trip.locations.findIndex((item) => item.id === locationId)
        return {
            locationIndex,
            location: trip.locations[locationIndex],
            nextLocation: locationIndex + 1 in trip.locations ? trip.locations[locationIndex + 1] : undefined,
            previousLocation: locationIndex - 1 in trip.locations ? trip.locations[locationIndex - 1] : undefined,
        }
    }, [params, trip])
                        ...
                        {selectedLocation?.location && (
                            ...
                            >
                                {selectedLocation.location.image && (
                                    <img style={{ maxWidth: "100%" }} src={selectedLocation.location.image} alt={selectedLocation.location.title} />
                                )}
                                <div className="location-popup-content">
                                    <strong>
                                        {selectedLocation.location.title} -{" "}
                                        {diffInDays(selectedLocation.location.visitStartDate, selectedLocation.location.visitEndDate)}
                                    </strong>
                                    <div>{selectedLocation.location.description}</div>
                                    <div>
                                        Last Visited On: <strong>{getTripDateLabel(selectedLocation.location.visitStartDate)}</strong>
                                    </div>
                                </div>
                            </Popup>
                        )}
